#!/usr/bin/env bash
# Runs the benchmark suite. Usage: run.sh <outputCsv> [warmups] [iterations]
set -e
cd "$(dirname "$0")/.."
OUT="${1:-benchmark/results/latest.csv}"
WARM="${2:-3}"
ITERS="${3:-5}"
...
java -Xms3g -Xmx3g -XX:+AlwaysPreTouch -cp "$CP" bench.Benchmark "$OUT" "$WARM" "$ITERS"
