...
| `src/bench/Benchmark.java` | 56 个场景：CNG 噪声 2D/3D/断裂链/Perlin、fit 选择、IRare 生物群系挑选（现代/legacy）、implode 稀有度（重建/缓存）、2D 插值×3、3D 插值×2、WorldCache2D（全 miss/命中）、逐列 RNG、地形列填充（legacy/新路径，真实 IrisBiome/Region/Dimension）、并行×3（8 线程共享 CNG/共享缓存散点/raster）、Matter/Mantle 存储层×4（DataContainer set/get、MantleChunk 写链、16³ Matter 序列化往返）、HyperLock×2（单线程命中、8 线程同键争用）、对象放置×2（IrisObject.place 主循环/STILT 循环，真实 IrisObject+IrisObjectPlacement+记录型 placer）、装饰器×2、沉积、洞穴雕刻×2、后处理、群系高度、铺层、**区块上下文预填充×2（新行任务路径 vs 旧协程编排复刻 `OldContextFill`——真实 kotlinx-coroutines）**、标志位提升、TectonicPlate 写读盘往返、**.iob 写入×3（R32：small/large/wide = 10k/100k/300k 块 × palette 12/32/96，用 bench 内 `FreshStringBlockData`——`getAsString()` 每次新分配字符串模拟 CraftBlockData，缓存实例桩会让 String.equals 走 `==` 快速路径从而伪造字符串扫描成本）**、**.iob 读取×3（R33：与写入场景同一组 fixture 序列化一次后反复读回——对象加载 KCache 缺载路径；digest 折叠 cursor 遍历的全部读回内容）**、**bget-parse（R34：B.get 咽喉点——16 个含状态串随机取用，量化解析 vs memo 命中）** |
| `src/bench/Verify3D.java` | 3D 适配器 vs 原 lambda 链的 60 万采样 A/B 等价性证明 |
| `src/bench/VerifyObjectIOB.java` | R13/R32 `.iob` 写入路径等价证明：重构后 `IrisObject.write()` vs 逐字复刻的 pre-R13 算法（addIfMissing+indexOf）全字节比对 + 读回往返；R32 起含跨 VectorMap chunk 用例（坐标 0/700/1024/1500/-1300 跨 4 chunk），证明 palette 构建（cursor）、写入（entrySet）、legacy 参考（values()）三种遍历跨 chunk 同序 |
...
- **全量运行时长与超时截断（2026-08-27，R31 记录）**：全套场景（3 预热+5 测量）在
  本机需 9-11 分钟，逼近外部工具 10 分钟超时；**被截断的 run 落盘不完整且无 "written" 行**
  （R31 复现：17 场景即止）。全量跑用 3 预热+3 测量（`run.sh <csv> 3 3`，verify 只用 it=0）
  压进时限，或拆分两半；验证 CSV 前先确认末行是 "written" 且场景数=56（R34 起；R33 为 55，R32 为 52，再前 49）。
