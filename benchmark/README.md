...
- **全量运行时长与超时截断（2026-08-27，R31 记录）**：全套场景（3 预热+5 测量）在
  ...
  压进时限，或拆分两半；验证 CSV 前先确认末行是 "written" 且场景数=55（R33 起；R32 为 52，再前 49）。
